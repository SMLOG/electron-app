var common = {
     ...
     * @ pricedigit： 最大的保留小数位
     */
    formatYNum: function (num, pricedigit) {
        var str = "";
        if (num > 100000 && num < 100000000) {
            var temp = (num / 10000).toFixed(pricedigit);
            if (temp >= 1000) {
                str = temp.substr(0, 4) + "万";
            } else {
                str = temp.substr(0, 5) + "万";
            }
        } else if (num > 100000000) {
            var temp = (num / 100000000).toFixed(pricedigit);
            str = temp.substr(0, 5) + "万";
        } else {
            if (num >= 1000 && pricedigit > 2) {
                str = num.toFixed(2);
            } else {
                str = num.toFixed(pricedigit);
            }
        }
        return str;
    },
    ...
    getElementByClass: function (ele, cls) {
        var res = [];  //存放匹配结果的数组
...
//////////////////
// WEBPACK FOOTER
// ./modules/chart/common/common.js
// module id = 40
// module chunks = 0