...
module.exports = function () {
    ...
    for (var i = 0, len = kall.length; i < len; i++) {
        ...
        if (item) {
            ...
            var o = item[1] / 1;        // open 开盘价
            var c = item[2] / 1;        // close 收盘价 (有可能是结算价， 还在商榷)
            var h = item[3] / 1;        // high 最高价
            var l = item[4] / 1;        // low 最低价
            var v = item[5] / 1;        // volume  成交量(股)
            var a = item[6] / 1;        // amount  成交额
...
//////////////////
// WEBPACK FOOTER
// ./modules/chart/web/k4/format/fk.js
// module id = 94
// module chunks = 0