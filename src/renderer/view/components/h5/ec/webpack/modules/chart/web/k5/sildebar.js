...
slideBar.prototype.drawFullData = function () {
    ...
    cc.EMLine(padding.left, ops.height - height - padding.bottom, padding.left + drawSumWdith, ops.height - padding.bottom);
    cc.restore();
...
slideBar.prototype.bindEvent = function () {
    ...
    document.addEventListener("mouseup", function (e) {
        ...
        if (moveCursor || moveBody) {
            ...
            _this.reClacRegion();
            console.timeEnd("www");
            kObj.options.onDragEnd(kObj.options.scale);
        }
...
slideBar.prototype.reClacRegion = function () {
    ...
    var start = Math.floor(dataAllSize * (cursorLeft / drawSumWdith));
    var end = Math.floor(dataAllSize * ((drawSumWdith - cursorRight) / drawSumWdith));
    if (end >= scale.min && end + 1 - start == scale.min) {
        start = end - scale.min;
    }

    if (end >= scale.fullDataSize) {
        end = scale.fullDataSize - 1;
    }
...
//////////////////
// WEBPACK FOOTER
// ./modules/chart/web/k5/sildebar.js
// module id = 105
// module chunks = 0