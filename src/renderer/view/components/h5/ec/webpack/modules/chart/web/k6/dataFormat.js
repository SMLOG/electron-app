    ...
    formatK: function () {
        ...
        if (flow.length > 0) {
            for (var i = 0; i < flow.length - 1; i++) {
                ...
                var end = flow[i + 1];
                var temp = {
                    start: new Date(start.time.substr(0,10).replace(/-/g, "/")).getTime(),
                    end: new Date(end.time.substr(0,10).replace(/-/g, "/")).getTime(),
                    ltg: start.ltg
                }
                flowTick.push(temp)
            }
            ...
            // var s = 
            flowTick.push({
                start: new Date(last.time.substr(0,10).replace(/-/g, "/")).getTime(),
                end: new Date(last.time.substr(0,10).replace(/-/g, "/")).getTime() + 999999999999,
                ltg: last.ltg
            });
...
//////////////////
// WEBPACK FOOTER
// ./modules/chart/web/k2/dataFormat.js
// module id = 166
// module chunks = 0