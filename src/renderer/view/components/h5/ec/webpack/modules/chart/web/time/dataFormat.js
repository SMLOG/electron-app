...
module.exports = function () {
    var dataAll = this.option.data;
    var time = dataAll.time;
    var datak = dataAll.datak;
    ...
    var yc = time.info.yc / 1;
    

    var info = {
        name: time.name,
        code: time.code,
        total: time.info.total / 1,
        jys: time.info.jys,
        yc: yc,
        ticks: tools.ticks(time.info.ticks || ""),
        iscrCount: tools.iscrCount((time.info.ticks || "")),
        decimal: null,         // 保留的小数位
        timeMax: yc,          // 分时最值
        timeMin: yc,
        ...
        avgMax: yc,           // 均线最值
        avgMin: yc,
        dayCount: -1,           // 一个交易日的数据量
        mk: time.info.mk
    }

    // // 盘前多15笔数据
    // if (this.option.type == "r" && (this.option.iscr == "true" || this.option.iscr == true)) {
    //     // info.total += 15;
    //     info.iscrCount = 15;
    // }


    var decimal = time.info.pricedigit + "";
...
//////////////////
// WEBPACK FOOTER
// ./modules/chart/web/time/dataFormat.js
// module id = 260
// module chunks = 0