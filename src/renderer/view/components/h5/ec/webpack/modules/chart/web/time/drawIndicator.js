...
drawIndicator.prototype.drawLine = function() {
  ...
  if (
    (this.o.options.iscr == "true" || this.o.options.iscr == true) &&
    this.o.options.before == false
  ) {
...
drawIndicator.prototype.drawScale = function() {
  ...
  for (var i = 0; i <= area.splity; i++) {
    var txt = tools.formatNumUnit(
      min + ((max - min) / area.splity) * (area.splity - i)
    );
    // txt = isNaN(txt) ? "0" : txt;
...
drawIndicator.prototype.getDDX = function(callback) {
  ...
  var methodName =
    ...
    new Date().getTime() +
    "_" +
    Math.floor(Math.random() * 8888888 + 1111111);
...
drawIndicator.prototype.tip = function(txt) {
  ...
  var urls = ["http://data.eastmoney.com/zjlx/{{code}}.html"];
