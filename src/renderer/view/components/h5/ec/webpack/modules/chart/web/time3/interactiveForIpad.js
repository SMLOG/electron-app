...
interactive.prototype.addEvent = function () {
    ...
    layerui.addEventListener("touchstart", function (e) {
        ...
        var right = ops.width - padding.right;
        var top = ops.areaTime.starty;
        var bottom = ops.height - padding.bottom + bcr.top - 18 - 6;
    ...
    layerui.addEventListener("touchmove", function (e) {
        ...
        if (data && data.isFoatmat && targetClass == "__ui" || tpClass == "__positionChanges" || tagName == "IMG") {
            ...
            if (x >= left && x < right && y >= top && y <= bottom) {
                ...
                if (_this.kObj.options.show.indicatorArea) {
                    try {
    ...
    layerui.addEventListener("touchend", function (e) {
        ...
        if (e.target.className == "__ui") {
            var te = e.targetTouches[0] || e.changedTouches[0];
        ...
        if (e.target.className.indexOf("timePositionChangesItem") == -1) {
            // 关闭盘口移动弹框
            var pkyd = layerui.querySelector(".__positionChangePop");
            if (pkyd) {
                layerui.removeChild(pkyd);
            }
        }
...
//////////////////
// WEBPACK FOOTER
// ./modules/chart/web/time3/interactiveForIpad.js
// module id = 327
// module chunks = 0