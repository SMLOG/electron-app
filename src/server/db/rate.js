...
var indexItems = [
  ...
  [
    ...
    "长期资金占重资产比率(%)",
    ">1",
    "长期资金占重资产比率 = (股东权益+非流动负债合计) / (固定资产 + 在建工程)",
  ],
  [
    "偿债能力",
    "流动比率(%)",
    ">3",
    "流动比率 = 流动资产总额 / 流动负债总额",
    `这个数值越大越好！流动比率越大，证明公司可以拿来还的流动资产越多。因此，好的公司不仅能还，还可以还很多！让银行、供应商放心的在家睡大觉，不要担心他们的投资血本无归
流动比率连续三年都大于300%最好。`,
  ],
  [
    "",
    ...
    `
    在流动资产中，有一部分是不能马上变现的，例如存货与预付账款，所以在计算速动比率时，要减掉存货和预付账款。
速动比率反映了公司的快速偿债能力`,
  ],
  ...
  [
    ...
    `净利润这个指标是可以调节的，所以是有缺陷的，所以那些基于净利润所计算出来的指标也是有缺陷的，比如ROE、ROA,所以 RO这RO那都有一定的局限性，这当然也包括了RORWA，所以一个单纯的RORWA比较不能说明问题，要动态的看，要全面的看。
    净利润取决于减值计提，减值计提取决于资产质量，所以要结合资产质量来分析`,
  ],
  [
    "",
    "扣非ROE(%)",
    "",
    "扣非ROE=  扣非净利率 * 总资产周转率 * 杠杆倍数",
    `净利润这个指标是可以调节的，所以是有缺陷的，所以那些基于净利润所计算出来的指标也是有缺陷的，比如ROE、ROA,所以 RO这RO那都有一定的局限性，这当然也包括了RORWA，所以一个单纯的RORWA比较不能说明问题，要动态的看，要全面的看。
    净利润取决于减值计提，减值计提取决于资产质量，所以要结合资产质量来分析`,
  ],
  [
  ...
  [
    ...
    "现金流量允当比率(%)",
    ">=1",
    "现金流量允当比率 = 最近5年度营业活动净现金流量 / ((五年内购建-五年内处置)+ (存货-五年前期初存货)+ 五年内分红)",
...
非流动负债合计=z.SUMNONLLIAB
固定资产=ifnull(z.FIXEDASSET,0)
在建工程=ifnull(z.CONSTRUCTIONPROGRESS,0)
工程物资=ifnull(z.CONSTRUCTIONMATERIAL,0)
现金=货币资金
约当现金=有价证券
流动资产总额=z.SUMLASSET
流动负债总额=z.SUMLLIAB
...
midItemMap["最近5年度利润"] = `( 
            select 
            sum(ifnull(l5.PARENTNETPROFIT,0) )
             from lrb l5 where 
             l5.code = ll.code 
             and l5.reporttype=ll.reporttype
             and l5.reportdate<=ll.rreportdate
             and l5.reportdate>DATE_FORMAT(DATE_SUB(STR_TO_DATE(ll.rreportdate,'%Y-%m-%d'),INTERVAL 5*4*3 MONTH),'%Y-%m-%d')
             )`;
midItemMap["五年前期初存货"] = `
           (select 
           z5.INVENTORY
...
getTrees(indexItems).map((tree) => {
  fs.writeFileSync(
    `${__dirname}/../../../static/${tree[0].id}.json`,
