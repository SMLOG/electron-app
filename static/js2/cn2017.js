...
var xh5_BrowserUtil = new function() {
    this.hdpr = function(e) {
        ...
        if (t.getContext && t.getContext("2d")) {
            return Math.ceil(window.devicePixelRatio || 1) / (t.getContext("2d").webkitBackingStorePixelRatio || 1)
        }
        return e.noH5 = !0,
        1
    }(this)
...
painterProp.resizeLoc = function() {
    ...
    switch (this.canvas.style.width = t + "px",
...
painterProp.newGStyle = function(e) {
    for (var t in e)
        e.hasOwnProperty(t) && (this.g[t] = e[t])
}
,
...
painterProp.newFillStyle_rgba = function(e, t, a) {
    for (var n = _g.createLinearGradient(0, 0, 0, t), i = 0, r = e.length; i < r; i++)
...
painterProp.clear = function(e, t) {
    ...
    this.canvas.width = a,
    e && (_c && _g.strokeStyle != _c && (_g.strokeStyle = _c),
    _g.beginPath()),
    this.scale(t)
}
,
painterProp.clearLimit = function(e, t) {
    this.g.clearRect(e, 0, t, this.canvas.height),
    this.g.beginPath()
}
,
...
tChartProp.loadFive = function() {
    ...
    jsonP({
        ...
        url: "//money.finance.sina.com.cn/quotes_service/api/openapi.php/CN_MarketData.getKLineData?symbol=" + paperCode + "&scale=240&ma=no&datalen=10",
        callback: !0,
        success: function(t) {
            var a = t.data;
            if (!a)
                return void (document.querySelector(".money-five") && (document.querySelector(".money-five").style.display = "none"));
            e.loadFiveZJ();
            for (var n = 0; n < a.length; n++)
                a[n].percent = 0 == n ? (1 * a[n].close - 1 * a[n].open) / (1 * a[n].open) : (1 * a[n].close - 1 * a[n - 1].close) / (1 * a[n - 1].close);
            a = a.reverse(),
            e.dataPFive = a.slice(0, 6)
        }
    })
}
,
tChartProp.loadFiveZJ = function() {
    ...
    jsonP({
        ...
        url: "//stock.sina.com.cn/stock/api/openapi.php/TouziService.getStockHistoryFlow?page=1&num=5&symbol=$symbol".replace("$symbol", paperCode),
        callback: !0,
        success: function(t) {
            if (t && t.data && t.data.data) {
                ...
                for (e.dataPFive[0].day.replace(/\//g, "-") != a[0].date && e.dataPFive.shift(),
                ...
                document.querySelector(".money-five") && (document.querySelector(".money-five").style.display = "none")
        }
    })
}
,
...
tChartProp.fiveListTemple = function(e) {
    for (var t = e, a = document.createElement("ul"), n = 0; n < t.data.length; n++) {
        ...
        case 1:
            ...
            break;
        case 2:
            i.innerHTML = Number(100 * t.data[n].percent) > 0 ? "+" + Number(100 * t.data[n].percent).toFixed(2) + "%" : Number(100 * t.data[n].percent).toFixed(2) + "%";
            break;
        case 3:
    ...
    new HQ.DataCenter({
        ...
        getObj: function(t) {
            ...
            e.zjData = {
                ...
                td_a: (Math.abs(1 * a.rxl_in_raw) || 0) - (Math.abs(1 * a.rxl_out_raw) || 0),
                td_r: a.rxl_net_rate || 0,
                d_i: Math.abs(1 * a.rl_in_raw) || 0,
                d_o: Math.abs(1 * a.rl_out_raw) || 0,
                d_a: (Math.abs(1 * a.rl_in_raw) || 0) - (Math.abs(1 * a.rl_out_raw) || 0),
                d_r: a.rl_net_rate || 0,
                z_i: Math.abs(1 * a.rm_in_raw) || 0,
                z_o: Math.abs(1 * a.rm_out_raw) || 0,
                z_a: (Math.abs(1 * a.rm_in_raw) || 0) - (Math.abs(1 * a.rm_out_raw) || 0),
                z_r: a.rm_net_rate || 0,
                x_i: Math.abs(1 * a.rs_in_raw) || 0,
                x_o: Math.abs(1 * a.rs_out_raw) || 0,
                x_a: (Math.abs(1 * a.rs_in_raw) || 0) - (Math.abs(1 * a.rs_out_raw) || 0),
                x_r: a.rs_net_rate || 0,
                zl_a: a.rp_net || 0,
            ...
            e.dataDetail = [Math.abs(1 * a.rxl_in_raw) || 0, Math.abs(1 * a.rxl_out_raw) || 0, Math.abs(1 * a.rl_in_raw) || 0, Math.abs(1 * a.rl_out_raw) || 0, Math.abs(1 * a.rm_in_raw) || 0, Math.abs(1 * a.rm_out_raw) || 0, Math.abs(1 * a.rs_in_raw) || 0, Math.abs(1 * a.rs_out_raw) || 0],
            e.dataDay = [e.zjData.td_a, e.zjData.d_a, e.zjData.z_a, e.zjData.x_a],
            e.renderChart(),
            e.cb({
                day: e.dataDay,
                zj: e.zjData
...
tChartProp.riseColorSetting = function() {
    window.riseColor && "riseGreen" == window.riseColor && (this.rise = this.bFall,
    this.fall = this.bRise)
}
,
tChartProp.dimDoubleTemple = function(e) {
    var t, a, n, i = this, r = e, o = r.series, s = 1;
    for (a = 0; a < o.length; a++)
        n = o[a].value,
        value = i["data" + n],
        i["max" + n] = Math.max.apply(null, value || 1),
        i["min" + n] = Math.min.apply(null, value || 1),
        i["abs" + n] = Math.max(Math.abs(i["max" + n]), Math.abs(i["min" + n])),
        i["min" + n] <= 0 && (s = 3);
    for (a = 0; a < o.length; a++)
        for (n = o[a].value,
        i["num" + n] = r.num,
        i["gapW" + n] = (i.mainW - 2 * i.paddingW) / i["num" + n],
        i["rectW" + n] = .4 * i["gapW" + n],
        i["zero" + n] = 0,
        i["dataH" + n] = [],
        i["dataY" + n] = [],
        t = 0; t < i["data" + n].length; t++) {
            var l, c, d = i["data" + n][t], p = Math.abs(i["data" + n][t]);
            s > 2 ? (l = p * i.chartH / (2 * i["abs" + n]),
            i["zero" + n] = i["abs" + n] * i.chartH / (2 * i["abs" + n]) + i.paddingH,
            c = d > 0 ? i["zero" + n] - l : i["zero" + n]) : (i["zero" + n] = i.chartH + i.paddingH,
            c = i["zero" + n] - p * i.chartH / i["abs" + n],
            l = p * i.chartH / i["abs" + n]),
            o[a].type && "line" == o[a].type && s > 2 && (c = d > 0 ? i["zero" + n] - l : i["zero" + n] + l),
            i["dataH" + n].push(l),
            i["dataY" + n].push(c)
        }
}
...
tChartProp.dimTemple = function(e, t) {
    ...
    a["rectW" + e] = .4 * a["gapW" + e],
    a["zero" + e] = 0,
    a["dataH" + e] = [],
    a["dataY" + e] = [];
    var n = 0;
    n = a["max" + e] < 0 ? a["min" + e] - a["max" + e] : a["max" + e] - a["min" + e];
    for (var i = 0; i < a["data" + e].length; i++) {
        var r, o, s = a["data" + e][i], l = Math.abs(a["data" + e][i]);
        r = l * a.chartH / n,
        a["min" + e] < 0 ? (a["zero" + e] = a["max" + e] * a.chartH / n + a.paddingH,
        o = s > 0 ? a["zero" + e] - r : a["zero" + e]) : (a["zero" + e] = a["max" + e] * a.chartH / a["max" + e] + a.paddingH,
        o = a["zero" + e] - l * a.chartH / a["max" + e],
        r = l * a.chartH / a["max" + e]),
        t.type && "line" == t.type && a["min" + e] < 0 && (o = s > 0 ? a["zero" + e] - r : a["zero" + e] + r),
        a["max" + e] < 0 && (a["zero" + e] = a.paddingH,
        o = a.paddingH,
...
tChartProp.detailRender = function() {
    var e = this.painterDatail
      , t = this.dataYDetail
      , a = this.dataHDetail;
    for (e.clear(!1, xh5_BrowserUtil.hdpr),
    i = 0; i < t.length; i++) {
        var n = void 0;
        i % 2 == 0 ? (n = this.rise,
        e.drawVStickRect((this.gapWDetail - this.rectWDetail) / 2 + 2 * this.paddingW / 5 + this.paddingW + i * this.gapWDetail, t[i], this.rectWDetail, a[i], n, !0)) : (n = this.fall,
        e.drawVStickRect((this.gapWDetail - this.rectWDetail) / 2 + this.paddingW + i * this.gapWDetail - 2 * this.paddingW / 5, t[i], this.rectWDetail, a[i], n, !0))
    }
    for (e.stroke(),
    e.newStyle("#ddd", !0, 1),
    ...
    e.newStyle("#fff", !0, 1),
    i = 0; i < t.length; i++)
        e.moveTo(this.paddingW + i * this.gapWDetail + this.rectWDetail / 2, t[i] - 1),
        e.lineTo(this.paddingW + i * this.gapWDetail + this.rectWDetail / 2, t[i] - 1),
        e.stroke(),
        e.g.textAlign = "center",
        e.g.fillStyle = "#333",
        e.g.fillText(unit(this.dataDetail[i], 0, 2), this.gapWDetail / 2 + this.paddingW + i * this.gapWDetail, this.dataDetail[i] >= 0 ? t[i] - 5 : t[i] + a[i] + 15),
        i % 2 == 0 && (e.g.textAlign = "start",
        e.g.fillText(this.param.labDetail[Math.floor(i / 2)], this.gapWDetail / 2 + 2 * this.paddingW / 5 + this.paddingW + i * this.gapWDetail, this.dataDetail[i] >= 0 ? this.zeroDetail + 15 : t[i] - 5));
    this.detailFlag || (this.detailFlag = !0,
    document.getElementById(this.param.domDetailCon).style.display = "none")
}
,
tChartProp.renderLine = function(e, t) {
    var a = this["painter" + t.painter]
      , n = this["dataY" + e];
    for (a.newStyle("orange", !0, 1),
    i = 0; i < n.length; i++)
        0 == i ? a.moveTo(this.paddingW + this["gapW" + e] / 2 + i * this["gapW" + e], n[i]) : a.lineTo(this.paddingW + this["gapW" + e] / 2 + i * this["gapW" + e], n[i]);
    for (a.stroke(),
    a.newStyle("orange", !0, .5),
    a.g.fillStyle = "orange",
    a.g.font = "12px bold Arial",
    a.newStyle("orange", !0, 1),
    i = 0; i < n.length; i++)
        a.drawDot(this.paddingW + this["gapW" + e] / 2 + i * this["gapW" + e], n[i], 2, !0),
        a.fill();
    a.stroke()
}
,
tChartProp.renderTemple = function(e, t) {
    var a = this["painter" + t.painter]
      , n = this["dataY" + e]
      , r = this["dataH" + e];
    for (a.clear(!1, xh5_BrowserUtil.hdpr),
    i = 0; i < n.length; i++) {
        var o = this["data" + e][i] > 0 ? this.rise : this.fall;
        a.drawVStickRect(this.paddingW + (this["gapW" + e] - this["rectW" + e]) / 2 + i * this["gapW" + e], n[i], this["rectW" + e], r[i], o, !0)
    }
    for (a.stroke(),
    a.newStyle("#ddd", !0, .5),
    ...
    a.newStyle("#fff", !0, 2),
    i = 0; i < n.length; i++)
        a.moveTo(this.paddingW + i * this["gapW" + e] + this["rectW" + e] / 2, n[i] - 1),
        a.lineTo(this.paddingW + i * this["gapW" + e] + this["rectW" + e] / 2, n[i] - 1),
        a.g.textAlign = "center",
        a.g.fillStyle = "#333",
        a.g.fillText(unit(this["data" + e][i], 0, 2), this.paddingW + i * this["gapW" + e] + this["gapW" + e] / 2, this["data" + e][i] > 0 ? n[i] - 5 : n[i] + r[i] + 15),
        "Five" == e || a.g.fillText(this.param.lab[i], this.paddingW + i * this["gapW" + e] + this["gapW" + e] / 2, this["data" + e][i] > 0 ? this["zero" + e] + 15 : n[i] - 5);
    a.stroke()
}
;
;function ZjTable(e) {
    ...
    this.BKUrl = "//gu.sina.cn/hq/api/openapi.php/StockV2Service.getPlateDetail?sort=rp_net&asc=0&page=1&num=1000&id=@bk&dpc=1",
    this.BKData = null,
    ...
    this.bkUrl = "https://money.finance.sina.com.cn/quotes_service/api/jsonp.php/var $cb=/Market_Center.getSymbolSW2?symbol=$symbol&source=apage&dpc=1",
    this.Sw = void 0,
    this.arrow = !0,
    this.addEvent(),
    this.loadBK()
...
zjTableProp.loadBKDetail = function() {
    ...
    document.getElementById(this.linkDom).href = this.linkMore.replace("@link", this.Sw),
    jsonP({
        varStr: "moneyBK",
        callback: !0,
        url: t,
        success: function(t) {
            t && t.data && (e.BKData = t.data,
            e.bkTemple(t.data),
            e.loadBkList())
        }
    })
...
zjTableProp.addEvent = function() {
    var e = this;
    document.getElementById("money_detail").addEventListener("click", function() {
        e.arrow ? (document.getElementById("money_detail_con").style.display = "block",
        document.getElementById("money_arrow_open").style.display = "none",
        document.getElementById("money_arrow_close").style.display = "block",
        "undefined" != typeof SUDA && SUDA.log()) : (document.getElementById("money_detail_con").style.display = "none",
        document.getElementById("money_arrow_open").style.display = "block",
        document.getElementById("money_arrow_close").style.display = "none"),
        e.arrow = !e.arrow
    })
}
,
zjTableProp.bkTemple = function(e) {
    ...
    var n = '<a href="' + this.linkMore.replace("@link", this.Sw) + '"><div class="tTitle"><span>\u6240\u5c5e\u884c\u4e1a</span><span class="money-blue" style="padding-left:.3rem;">' + t.name + '</span></div></a><ul><li class="line"><span>\u6da8\u8dcc\u5e45</span><span class="value ' + c + '">' + (100 * Number(t.avgRate)).toFixed(2) + '%</span></li><li class="line right"><span>\u4e3b\u529b\u51c0\u6d41\u5165</span><span class="value">' + unit(t.totalRp_net, 2) + '</span></li></ul><ul><li class="line bottom"><span>\u6da8\u8dcc\u6570</span><span class="value"><span class="clearM ' + this.rise + '">' + t.leadNum + '</span>/<span class="clearM ' + this.fall + '">' + t.ledNum + '</span></span></li><li class="line bottom right"><span>\u6210\u4ea4\u989d</span><span class="value">' + unit(t.totalAmount, 2) + "</span></li></ul>";
...
zjTableProp.loadBkList = function() {
    ...
    for (n = 0; n < a.length; n++)
        a[n] == paperCode && (i = 1);
    0 == i && (a.splice(2, 0, paperCode),
    t.bkListIndex.splice(2, 0, r || "--")),
    a = a.join(","),
    new HQ.DataCenter({
        symbols: a,
        isANeedCWZJ: !0,
...
zjTableProp.bkListTemple = function(e) {
    ...
    for (var i in t)
        if (t.hasOwnProperty(i)) {
            var r, o = t[i], s = this.colorSetting(o.type), l = (1 * o.rxl_in_raw || 0) - (1 * o.rxl_out_raw || 0) + (1 * o.rl_in_raw || 0) - (1 * o.rl_out_raw || 0), c = o.code == paperCode ? "money-cur-back" : "", d = 0;
            r = l > 0 ? "red" : l < 0 ? "green" : "equal",
            d = this.colorSetting(r),
            a += '<ul class="rank-data ' + c + '"><a href="' + "//quotes.sina.cn/hs/company/quotes/view/$symbol".replace("$symbol", o.code) + '"><li>' + this.bkListIndex[n] + '</li><li style="line-height: 0;"><p class="name size">' + o.name + '</p><p class="name code ' + s + '">' + o.price + " " + o.percent + '</p></li><li class="' + d + '">' + unit(l, 0, 2) + "</li><li>" + unit(o.totalAmount_i, 0, 2) + "</li></a></ul>",
            n++
        }
...
zjTableProp.zjTemple = function(e) {
    var t = e
      , a = '<ul><li class="line"><span>\u4e3b\u529b\u51c0\u6d41\u5165</span><span class="value">' + unit(Number(t.d_a) + Number(t.td_a), 2) + '</span></li><li class="line right"><span>\u6210\u4ea4\u989d</span><span class="value">' + unit(t.totalAmount, 2) + '</span></li></ul><ul><li class="line bottom"><span>\u4e3b\u529b\u51c0\u6d41\u5165\u7387</span><span class="value">' + t.zl_r + '</span></li><li class="line bottom right"><span>\u6362\u624b\u7387</span><span class="value">' + t.turnOver + "</span></li></ul>";
    this.zjDom.innerHTML = a,
    this.ctmHtml()
}
,
    ...
    if (this.hasCtm && !(this.hasCtm > 1)) {
        ...
        if (e) {
            var t = {
                ...
                androidInstallUrl: "http://file.finance.sina.com.cn/finapp/apks/sinafinance_slhqbanner.apk",
                needOpenSource: !1
            }
              , a = new SinaFinanceCallUp.CallUpSinaFinance(t)
              , n = document.createElement("div");
            n.className = "ctm",
            n.innerHTML = '<p class="text">\u8d22\u7ecf\u5ba2\u6237\u7aef\u8d44\u91d1\u6d41\u5411\u6570\u636e\u66f4\u51c6\u786e\u3001\u66f4\u65b0\u66f4\u53ca\u65f6\u3002<span>\u70b9\u6b64\u67e5\u770b</span>\u3002</p>',
            e.appendChild(n),
...
;function jsonP(e) {
    ...
    a.onload = a.onreadystatechange = function() {
        this.readyState && "loaded" !== this.readyState && "complete" !== this.readyState || (r || i(),
    ...
    ,
    n.appendChild(a),
    r && (window[t] = function(e) {
        i && i(e.result ? e.result : e),
        delete window[t]
    }
    )
}
function loader(e, t, n, a) {
    ...
    r.onload = r.onreadystatechange = function() {
        i || r.readyState && !/loaded|complete/.test(r.readyState) || (i = !0,
    ...
    ,
    o.parentNode ? o.parentNode.insertBefore(r, o) : l ? s.insertBefore(r, l) : s.appendChild(r)
}
!function(e, t) {
    function n(e) {
    ...
    var i, r, o, s, l, c, d, p, h, m, f = "", u = {
        Webkit: "webkit",
        Moz: "",
        O: "o"
    }, _ = document.createElement("div"), v = /^((translate|rotate|scale)(X|Y|Z|3d)?|matrix(3d)?|perspective|skew(X|Y)?)$/i, g = {};
    _.style.transform === t && e.each(u, function(e, n) {
    ...
    r = f + "transform",
    g[o = f + "transition-property"] = g[s = f + "transition-duration"] = g[c = f + "transition-delay"] = g[l = f + "transition-timing-function"] = g[d = f + "animation-name"] = g[p = f + "animation-duration"] = g[m = f + "animation-delay"] = g[h = f + "animation-timing-function"] = "",
    e.fx = {
    ...
    e.fn.animate = function(n, a, i, r, o) {
        return e.isFunction(a) && (r = a,
        i = t,
        a = t),
        e.isFunction(i) && (r = i,
        i = t),
        e.isPlainObject(a) && (i = a.easing,
        r = a.complete,
        o = a.delay,
        a = a.duration),
        a && (a = ("number" == typeof a ? a : e.fx.speeds[a] || e.fx.speeds._default) / 1e3),
        o && (o = parseFloat(o) / 1e3),
        this.anim(n, a, i, r, o)
    }
    ,
    e.fn.anim = function(a, i, f, u, _) {
        var y, b, w, C = {}, k = "", x = this, S = e.fx.transitionEnd, T = !1;
        ...
        "string" == typeof a)
            ...
            b = [];
            for (y in a)
                v.test(y) ? k += y + "(" + a[y] + ") " : (C[y] = a[y],
                b.push(n(y)));
            k && (C[r] = k,
            b.push(r)),
            i > 0 && "object" == typeof a && (C[o] = b.join(", "),
            C[s] = i + "s",
            C[c] = _ + "s",
            C[l] = f || "linear")
        }
...
!function(e) {
    function t() {
        var e = 0
          ...
          , s = document
          , l = s.documentElement;
        return e = o.innerWidth || l.clientWidth || s.body.clientWidth || 0,
    ...
    function n(e, t, n, a, i, r) {
        void 0 === t && (t = "-");
        var o = [];
        if (a || o.push(e[n ? "getUTCFullYear" : "getFullYear"]()),
        !i) {
    ...
    function s(t) {
        function a() {
            ...
            e.ajax({
                ...
                dataType: "jsonp",
                success: function(t) {
                    t && (l.data = t.result.data,
                    l.pageNum++,
                    o(e("#" + l.param.dom), l.data),
                    e("#cn_news_cont").find("a").each(function(t, n) {
        ...
        function i() {
            ...
            loader(L.commentUrl.replace("$symbol", n) + L.varComment + "=" + t, function() {
                var t, a = window[L.varComment];
                if (l.data = a,
                l.data) {
                    if (!a.data)
                        return e("#cn_comment_refresh").hide(),
                        e("#cn_comment_more").hide(),
                        t = L.nothingComment.replace("@code", n),
                        void e("#" + l.param.dom).find("ul").append(t);
                    if (0 == a.data.threads) {
                        if (e("#cn_comment_refresh").hide(),
                        e("#cn_comment_more").hide(),
                        l.isMore) {
                            var i = L.nothingCommentMore.replace("@code", n);
                            return void e("#" + l.param.dom).find("ul").append(i)
                        }
                        return t = L.nothingComment.replace("@code", n),
                        void e("#" + l.param.dom).find("ul").append(t)
                    }
                    l.isMore = 1,
                    l.relate_tid = a.data.threads[a.data.threads.length - 1].tid,
                    l.relate_value = a.data.threads[a.data.threads.length - 1].timestamp,
                    r(e("#" + l.param.dom), l.data)
                }
            })
        }
        function r(t, n) {
            for (var a = t.find("ul")[0], i = n.data.threads, r = i.length, o = 0; o < r; o++) {
                var s = L.guHtml
                  , l = i[o]
                  , c = document.createElement("li")
                  , d = l.content.replace("<br />", "")
                  , p = l.title ? l.title : d.length < 30 ? d : d.substring(0, 30) + "..."
                  , h = f = l.lastctime.split(" ")
                  , m = h[0].substring(5, h[0].length)
                  , f = h[1].substring(0, 5);
                s = s.replace("@content", p).replace("@src", l.user.portrait).replace("@time", m + " " + f).replace("@nick", l.user.nick).replace("@url", "//guba.sina.cn/view_" + l.bid + "_" + l.tid + ".html"),
                c.innerHTML = s,
                e(a).append(c)
            }
            e("#cn_comment_more").show(),
        ...
        function o(t, a) {
            var i = t.find("ul")[0]
              ...
              , o = r.length;
            if (0 == o) {
                if (e("#cn_news_more").hide(),
                e("#cn_news_refresh").hide(),
                l.isMore)
            ...
            for (var c = 0; c < o; c++) {
                ...
                p.thumbs && p.thumbs.length > 0 && (h = "",
                m = p.thumbs[0].replace("http://", "https://"),
                d = d.replace("@dlhg", "1.6rem").replace("@ddhg", "1.2rem").replace("@h3hg", ".8rem")),
                d = d.replace("@content", p.waptitle).replace("@source", p.media).replace("@time", u).replace("@src", m).replace("@display", h).replace("@href", p.wapurl),
                f.innerHTML = d,
                e(i).append(f)
            }
            e("#cn_news_more").show(),
    ...
    function h(e) {
        this.param = e,
        this.url = "//quotes.sina.cn/extra/api/openapi.php/RelateStockService/getStocks?symbol=$symbol",
        this.rData = null,
        this.symbols = "",
        this.flag = !1,
        this.data = [],
    ...
    function u(e, t) {
        var n, a = "";
        return e > 1e9 ? (n = (e / 1e8).toFixed(0),
        a = "\u4ebf") : e > 1e7 ? (n = (e / 1e7).toFixed(0),
        a = "\u5343\u4e07") : e > 1e5 ? (n = (e / 1e4).toFixed(0),
        a = "\u4e07") : n = e >= 1 ? e.toFixed(0) : "-",
        t ? n + a : n
    }
    function _() {
        this.addEvent(),
    ...
    function v() {
        function t(e) {
            var t, n = document.createElement("div"), a = "http://m.news.leju.com/stock/" + paperCode + ".html#source=m_sina_cjgs&source_ext=sina", i = '<li class="cn-stock-flex"><a suda-uatrack="key=hq_center_hs&value=leju_company" href=' + a + ">\u516c\u53f8</a></li>";
            n.innerHTML = i,
            t = n.childNodes[0],
            e.appendChild(t),
        ...
        function a() {
            N || (N = new m({
                ...
                var a = window.bkSw && window.bkSw[0] && window.bkSw[0].type
                  , i = document.getElementById("cn_news_tab_con").querySelector(".cn-tab")
                  , r = document.getElementById("cn_f_con").querySelector(".cn-tab");
                "sw2_430100" === a || "sw2_430200" === a ? (t(i),
                t(r)) : (n(i),
                n(r))
            }
            )),
        ...
        function _(t) {
            ...
            case "report":
                ...
                break;
            default:
                I.lastDom && (e("#cn_news_tab_con").find("li")[j].innerHTML = I.activeDom,
            ...
            W.addEvent(),
            "data" == hashValue[2] && H(),
            "relate" == hashValue[2] && (z && z.addBlank(),
            a()),
            "news" == hashValue[2] && i(),
            "money" == hashValue[2] && u(),
            X = e("#cn_head").offset().height,
            1 == K && (Z = e("#" + O).offset().top - X + 2,
            window.scrollTo(0, Z)),
            I.flag = !0,
            I.fFlag = !0,
            I.close(j),
        ...
        function b() {
            e("#cn_news_more").on("click tap", function() {
                g({
                    pos: "callup_morenews",
                    androidurl: "http://file.finance.sina.com.cn/finapp/apks/sinafinance_waphangqing.apk"
                })
            }),
            ...
            e("#cn_comment_tw").on("click tap", function() {
                var e = "//guba.sina.cn/list_$symbol.html".replace("$symbol", paperCode);
                window.open(e, "_self")
            }),
        ...
        function C() {
            ...
            e(window).scroll(function() {
                ...
                e("#fix_header").hide(),
                e("#fix_floating").show(),
                e("#fix_floating").find("header").css("position", ""),
                I.close(j),
                I.flag = !0) : (K = 0,
                e("#fix_floating").hide(),
                n > J && e("#fix_header").show(),
                I.fFlag = !0,
                I.fClose(j)) : (e("#fix_floating").hide(),
                e("#fix_header").hide(),
                I.fFlag = !0,
                I.fClose(j)),
                k(),
                F && (Q = e("#cn_bk_hy_c").offset().top,
                F.flag = !1,
                document.querySelector("." + F.param.pop).style.display = "none",
                Q <= n + window.innerHeight && !F.hasExposure && F.hasNews && F.sima())
            })
        }
        function k() {
            0 == K ? e(".cn-expert-tw").css("display", "none") : e(".cn-expert-tw").css("display", "block")
        }
        function S() {
            new HQ.DataBox({
                isANeedPHP: !1,
        ...
        function H() {
            ...
            t.css("height", 0);
            var a = window.getComputedStyle(document.getElementById("cn_data_cont"), null).height
        ...
        var E, N, M, F, U, q, B, A, P, z, R = ["cn_tab_relate", "cn_tab_news", "cn_tab_comment", "cn_tab_money", "cn_tab_finance", "cn_tab_f10", "cn_tab_notice", "cn_tab_report"], $ = ["cn_relate_cont", "cn_news_cont", "cn_comment_cont", "cn_money_cont", "cn_finance_cont", "cn_f10_cont", "cn_notice_cont", "cn_report_cont"], G = ["cn_tab_relate_f", "cn_tab_news_f", "cn_tab_comment_f", "cn_tab_money_f", "cn_tab_finance_f", "cn_tab_f10_f", "cn_tab_notice_f", "cn_tab_report_f"], O = "cn-news-c", K = 0, V = "hq_userColor", Z = e("#" + O).offset().top, X = e("#cn_head").offset().height;
        W = new c({
            tab: R,
            con: $,
            tabF: G,
    ...
    function y() {
        return window.navigator.userAgent.toLowerCase().match(/MicroMessenger/i) == J
    }
    function b() {
    ...
    function k() {
        ...
        t.on("click", function(e) {
            ...
            if ("navstock" !== t && "navpoint" !== t) {
                ...
                "undefined" == typeof SinaFinanceCallUp)
                    throw new Error("nil SinaFinanceCallUp");
                new SinaFinanceCallUp.CallUpSinaFinance(te).tryDirectCall(ne[t].callopts)
            }
        }),
        n.on("click", "a", function(e) {
            ...
            if ("news" !== t && "relate" !== t) {
                if ("undefined" == typeof SinaFinanceCallUp)
                    throw new Error("nil SinaFinanceCallUp");
                new SinaFinanceCallUp.CallUpSinaFinance(te).tryDirectCall(ne.tabcall.callopts)
            }
        }),
    ...
    var x = {
        ...
        get: function(e) {
            var t = document.cookie.match("(?:^|;)\\s*" + this.escape(e) + "=([^;]*)");
            return t ? t[1] || "" : ""
        },
        set: function(e, t, n) {
            !n && (n = {}),
    ...
    S.setMCookie = function(e) {
        var t = e.list
          ...
          , a = e.prefix
          , i = this;
        t ? (t = t.replace(this.code + ",", ""),
        t = t.replace(this.code, ""),
        t = i.code + "," + t,
        "," == t.charAt(t.length - 1) && (t = t.substring(0, t.length - 1))) : t = i.code;
        var r = t.split(",");
        r.length > n && r.pop(),
        t = r.toString(),
        x.set(a, t, {
    ...
    D.resizeChart = function(n) {
        var a = this
          ...
          , l = e(".cn-chart");
        setTimeout(function() {
            a.resize() ? (e("#fix_floating").hide(),
            e("#fix_header").hide(),
            window.scrollTo(0, 0),
            s.hide(),
            r.show().height(i.viewHeight + "px").append(o),
            n.setDirection("horizontal"),
            SUDA.uaTrack("hq_center_hs", "horizontal")) : (s.show(),
            r.hide(),
            l.append(o),
            n.setDirection("vertical"))
        }, 100)
    }
    ,
    D.resize = function() {
    ...
    var L = {
        ...
        newsUrl: "//cj.sina.cn/api/ct_news/get_news?market=cn&symbol=$symbol&su2cu=1&page=$page&num=10&fr=wap&mi=1",
        commentUrl: "//guba.sina.com.cn/api/?s=h5bar&dpc=1&bname=$symbol.cn&jsonpflag=1&callback=var ",
        guBaUrl: "//guba.sina.com.cn/api/?s=h5bar&bname=$symbol.cn&dpc=1",
        bkUrl: "https://money.finance.sina.com.cn/quotes_service/api/jsonp.php/var $cb=/Market_Center.getSymbolSW2?symbol=$symbol&source=apage&dpc=1",
        gnUrl: "https://money.finance.sina.com.cn/quotes_service/api/jsonp.php/var $cb=/Market_Center.getSymbolCHGN?symbol=$symbol&source=apage&dpc=1",
        detailUrl: "//vip.stock.finance.sina.com.cn/quotes_service/view/CN_TransListV2.php?num=10&symbol=$symbol&dpc=1&" + 1e3 * Math.random(),
        varBK: "cnBk",
        varGN: "cnGn",
        varNews: "cnNews",
        varData: "cnData",
        ...
        positionHtml: function(e) {
            var t = e
              , n = e.color
              , a = Number(t.price) ? "<li style=color:" + n + ">" + t.price + "</li>" : "<li>--</li>"
              , i = Number(t.volume) ? "<li>" + t.volume + "</li>" : "<li>--</li>";
            return '<li><ul class="cn-pos" style="border-top:' + t.style + '"><li>' + t.time + "</li>" + a + i + "</ul></li>"
        },
        detailHtml: function(e) {
            var t = e
              ...
              , a = e.bsColor
              , i = e.type;
            return '<li><ul class="cn-detail"><li>' + t.time + '</li><li style="color:' + n + '">' + t.price + "</li><li>" + t.volume + '<span style="color:' + a + '">' + i + "</span></li></ul></li>"
        },
        bkHtml: function(e, t) {
            var n = e
              , i = n.type
              , r = n.percent > 0 ? "+" + n.percent.toFixed(2) + "%" : n.percent.toFixed(2) + "%"
              , o = n.lead_cname
              , s = n.increase > 0 ? "+" + n.increase.toFixed(2) + "%" : n.increase.toFixed(2) + "%"
              , l = a(n.percent, L.cssClass)
              , c = a(n.increase, L.cssClass);
            return '<ul class="cn-bk"><li><a href="//gu.sina.cn/m/#/stock/blockdetail?id=' + i + '"><p>' + n.name + '</p></a></li><li><a href="//gu.sina.cn/m/#/stock/blockdetail?id=' + i + '"><p style="color:' + l + '">' + r + '</p></a></li><li><a href="//quotes.sina.cn/hs/company/quotes/view/' + n.lead_shares + '/"><span>' + o + '</span><span style="color:' + c + '">' + s + "</span></a></li></ul>"
        },
        follow: function(e) {
            var t = e
              , n = t.title.length > 20 ? t.title.substring(0, 19) + "..." : t.title;
            return '<li class="cn-panel"><a href="' + t.stockUrl + '"><div class="name">' + t.name + '</div><div class="code">' + t.code.toUpperCase() + '</div><div class="price" style="color:' + t.color + '">' + t.price + '</div><div><span class="diff" style="color:' + t.color + '">' + t.change + '</span><span class="percent" style="color:' + t.color + '">' + t.percent + '</span></div><div></a><span class="title">\u4e8b\u4ef6</span><a href="' + t.news_url + '"><span class="content">' + n + "</span></a></div></li>"
        },
        tradeMore: '<div class="trade-more"><a class="trade-more-list" style="display: none;" href="//dp.sina.cn/dpool/stock_new/v2/cjmx.php?code=' + paperCode + '&page=1"><div>\u67e5\u770b\u6210\u4ea4\u660e\u7ec6 ></div></a><a class="trade-more-list" style="display: none;" href="//dp.sina.cn/dpool/stock_new/v2/stock_history.php?code=' + paperCode + '"><div>\u67e5\u770b\u5386\u53f2\u4ea4\u6613 ></div></a></div>',
        nothing: '<div class="cn-nothing"><div></div><span><a style="color: #0090f7;" href="http://finance.sina.cn/roll.d.html?vt=4&pos=102&cid=76524&rollCid=76524">\u6682\u65e0\u4e2a\u80a1\u8d44\u8baf\uff0c\u70b9\u51fb\u67e5\u770b\u6caa\u6df1\u6eda\u52a8\u65b0\u95fb</a></span></div>',
        nothingComment: '<div class="cn-nothing"><div></div><span><a style="color: #0090f7;" href="//guba.sina.cn/list_@code.html">\u6682\u65e0\u6570\u636e \u70b9\u51fb\u524d\u5f80\u8bc4\u8bba</a></span></div>',
        nothingMore: '<div style="display: inline-block; text-align: center;width: 100%;padding: .3rem 0;background: #f8f8f8;"><a style="color: #0090f7;" href="http://finance.sina.cn/roll.d.html?vt=4&pos=102&cid=76524&rollCid=76524">\u6682\u65e0\u66f4\u591a\u6570\u636e \u70b9\u51fb\u67e5\u770b\u6caa\u6df1\u6eda\u52a8\u65b0\u95fb</a></div>',
        nothingCommentMore: '<div style="display: inline-block; text-align: center;width: 100%;padding: .3rem 0;background: #f8f8f8;"><a style="color: #0090f7;" href="//guba.sina.cn/list_@code.html">\u6682\u65e0\u66f4\u591a\u6570\u636e \u70b9\u51fb\u524d\u5f80\u8bc4\u8bba</a></div>',
    ...
    E.hqComponent = function(e) {
        ...
        if (e) {
            ...
            n = r.join(",");
            var o = e.data;
            if ("--" == o.price && "--" == o.prevclose && "--" == o.name && "--" == o.totalVolume && "--" == o.open || t.param.visitObj.setVisit(),
            !n)
                return t.param.dom.hide(),
                !0
        } else
            n = t.relateSymbol;
        "" != n && new HQ.DataCenter({
    ...
    E.relatedRender = function(t) {
        var n = this;
        n.inited || (n.inited = !0);
        for (var a = n.param.dom.find("li")[0], i = "", r = 0; r < t.length; r++) {
            var o, s = L.relatedHtml, l = t[r], c = l.name, d = L.cssClass.themeGreen, p = L.cssClass.themeRed;
            switch (l.type) {
            case "green":
                o = d;
                break;
            ...
            s = s.replace("@price", l.price).replace("@zdf", l.percent).replace("@href", l.url).replace("@symbol", String(l.code).toUpperCase()).replace("@name", c).replace("@color", o).replace("@attr", String(l.code).toUpperCase()),
            i += s
        }
        e(a).append(i),
        n.addBlank()
            ...
            if (e.getAttribute("data-attr"))
                for (var n = 0; n < t.length; n++) {
                    ...
                    if (a.code.toUpperCase() == e.getAttribute("data-attr")) {
                        ...
                        e.nextElementSibling.childNodes[0].className = "cn-relate-color cn-relate-@color".replace("@color", a.type);
                        var i = "";
                        i = "red" == a.type ? L.cssClass.themeRed : "green" == a.type ? L.cssClass.themeGreen : L.cssClass.themeEqual,
                        e.nextElementSibling.childNodes[0].style.backgroundColor = i
                    }
                }
        }
        for (var a = e("#cn_relate_cont").find("li"), i = a.length, r = e("#cn_visit_cont").find("li"), o = r.length, s = 0; s < i; s++) {
            var l = a[s];
            n(l, t)
        }
        for (s = 0; s < o; s++)
    ...
    N.setCss = function(t, n) {
        ...
        if (a.hide(),
        "undefined" != typeof SUDA)
            if ("cn_tab_finance" == t.target.id || "cn_tab_finance_f" == t.target.id) {
    ...
    M.title = function(e) {
        var t = e.code;
        document.title = e.name + " " + t.toUpperCase() + " " + e.price + "(" + e.change + " " + e.percent + ")- \u65b0\u6d6a\u8d22\u7ecf"
    }
    ,
    M.render = function(e) {
        var t = this
        ...
        0 == t.inited && (t.tempHtml = n.innerHTML,
        t.tempHHtml = a.innerHTML,
        t.inited = 1);
        var i, r = t.tempHtml, o = t.tempHHtml, s = e, l = L.cssClass.themeGreen, c = L.cssClass.themeRed;
        switch (s.type) {
        case "green":
            i = l;
            break;
        ...
        r = r.replace("@name", s.sname).replace("@price", s.price).replace("@zdf", s.percent),
        o = o.replace("@name", s.sname).replace("@price", s.price).replace("@zdf", s.percent),
        a.innerHTML = o,
        n.innerHTML = r;
        for (var d = 0; d < 2; d++)
        ...
        for (var e = this, t = 0; t < e.dom.length; t++)
            e.dom[t].on("touchend", function() {
                if (e.isMove) {
                    ...
                    e.transformX[0] = e.transformX[1] = 0) : t[0] < e.screeWidth - this.offsetWidth ? (e.animationDom({
                        dom: e.param.dom[0][0],
                    ...
                    e.transformX[0] = e.transformX[1] = e.screeWidth - this.offsetWidth) : e.transformX[0] = e.transformX[1] = 1 * t[0]
                }
            })
    }
    ;
    ...
    U.sima = function() {
        var e = this;
        e.inited && e.hasNews && !e.hasExposure && (void 0 !== window.SIMA && window.SIMA({
            action: "_exposure",
            data: {
                aid: "hq_center_hs",
    ...
    U.clearedUp = function() {
        ...
        if (t.param.parentNode) {
            ...
            if (a < 2)
                return void (t.param.parentNode.style.display = "none");
            t.hasNews = !0,
            t.param.parentNode.style.display = "block";
            for (var i = 0; i < a; i++) {
                var r = e[i]
                  , o = {
                    name: r.r_name,
    ...
    U.render = function() {
        ...
        e.param.dom.parentNode.style.width = 4.3 * t + .3 + "rem",
    ...
    q.load = function() {
        ...
        loader(t.param.url.replace("$symbol", paperCode).replace("$cb", this.param.varStr + n), function() {
            var a = window[t.param.varStr + n]
              , i = a;
            i ? (i.length <= 0 ? e("#" + t.param.pDom).hide() : t.render(i),
            "cn_bk_hy" == t.param.dom && t.cb && t.cb(i)) : e("#" + t.param.pDom).hide()
        }, function() {
            e("#" + t.param.pDom).hide(),
            console.log("error...")
        })
    ...
    ,
    q.render = function(e) {
        for (var t = e, n = t.length, a = this, i = document.getElementById(a.param.dom), r = 0; r < n; r++) {
            var o = document.createElement("li");
            o.innerHTML = L.bkHtml(t[r], a.param),
            i.appendChild(o)
        }
    }
    ;
    ...
    B.load = function(e) {
        var t = this;
        e[t.param.symbol] && (t.hqData = e[t.param.symbol],
    ...
    B.updateRender = function() {
        var e = this
          ...
          , p = t.sellNumArr
          , h = function(e) {
            var n, i = "--", r = Number(e.price) ? e.price : i, o = Number(e.volume) ? u(Number(e.volume), !0) : i;
            return r !== i && (n = a(e.price - t.prevclose, L.cssClass)),
            {
                price: r,
                volume: o,
                color: n
        ...
        for (i = 0; i < d; i++) {
            ...
            n.childNodes[4 - i].childNodes[0].childNodes[2].innerHTML = m.volume,
            n.childNodes[4 - i].childNodes[0].childNodes[1].style.color = m.color
        }
        for (i = 0; i < l; i++) {
            var f = h({
                price: o[i],
            ...
            n.childNodes[i + 5].childNodes[0].childNodes[2].innerHTML = f.volume,
            n.childNodes[i + 5].childNodes[0].childNodes[1].style.color = f.color
        }
        var _ = e.hqData.tradeItems
          , v = _.length >= 10 ? 10 : _.length;
        for (i = 0; i < v; i++) {
            r.childNodes[i].childNodes[0].childNodes[0].innerHTML = _[i] ? String(_[i][0]).substring(0, 5) : "",
            r.childNodes[i].childNodes[0].childNodes[1].innerHTML = _[i] ? Number(_[i][2]).toFixed(2) : "",
            r.childNodes[i].childNodes[0].childNodes[2].innerHTML = _[i] ? (1 * _[i][1] / e.u).toFixed(0) : "";
            ...
            r.childNodes[i].childNodes[0].childNodes[1].style.color = a(Number(_[i][2] - t.prevclose), L.cssClass)
        }
    }
    ,
    B.render = function() {
        ...
        for (r = 0; r < m; r++)
            ...
            o.bsColor = h[r] ? "UP" == h[r][3] ? L.cssClass.themeRed : L.cssClass.themeGreen : "",
    ...
    A.fOpen = function(e) {
        ...
        this.lastDomf && (this.cn_f_con.find("li")[this.index].innerHTML = this.lastDomf,
    ...
    A.fClose = function(e) {
        ...
        this.cn_f_open.hide(),
        this.index = void 0 !== e ? e : this.index,
        this.lastDomf && (this.cn_f_con.find("li")[this.index].innerHTML = this.activeDomf,
    ...
    A.open = function(t) {
        ...
        this.lastDom && (this.cn_news_tab_con.find("li")[this.index].innerHTML = this.activeDom,
    ...
    A.close = function(t) {
        ...
        this.index = void 0 !== t ? t : this.index,
        this.lastDom && (this.cn_news_tab_con.find("li")[this.index].innerHTML = this.lastDom,
    ...
    X.on("click", ".report-table", Q),
    Y.on("click", ".summary-table", Q);
    var J = "micromessenger"
      , ee = ["google.com", "bing.com", "baidu.com", "sogou.com", "so.com", "easou.com"]
      , te = {
        eventid: "hq_center_hs",
        uatrackKey: "hq_center_hs",
        subname: "outrefer2app",
        needOpenSource: !1,
        androidInstallUrl: "http://file.finance.sina.com.cn/finapp/apks/sinafinance_sousuo.apk"
    }
      , ne = {
        navmystock: {
            callopts: {
...
function(e) {
    ...
    function t(t) {
        return this.each(function() {
            ...
            i || (a.data("infiniteScroll", "mounted"),
            i = new n(a,r)),
            r.interval && i.infinite()
        })
    }
    if (__isNewsApp)
        return document.getElementById("cn_head").style.display = "none",
    ...
    n.DEFAULT = {
        interval: 8e3,
        service: "//quotes.sina.cn/extra/api/openapi.php/CalendarService.getLatestIndex?size=5"
    },
    n.prototype = {
        constructor: n,
        remoteData: function() {
        ...
        render: function(e) {
            ...
            e.forEach(function(e, n) {
                t += '<div class="calendar-scroll-item" data-order="@order"><p class="item-title">@country-\u516c\u5e03@title\u524d\u503c@previous\u9884\u6d4b\u503c@median\u516c\u5e03\u503c@ifr_actual</p><div class="clearfix item-msg"><span class="msg-datetime">@datetime </span><span class="msg-mark"><span style="vertical-align: middle">\u91cd\u8981\u6027:</span>@mark</span></div></div>'.replace("@order", n).replace("@title", e.event).replace("@datetime", e.date + " " + e.time).replace("@country", e.country).replace("\u524d\u503c@previous", e.previous ? ": \u524d\u503c" + e.previous : "").replace("\u9884\u6d4b\u503c@median", e.median ? ": \u9884\u6d4b\u503c" + e.median : "").replace("\u516c\u5e03\u503c@ifr_actual", e.ifr_actual ? ": \u516c\u5e03\u503c" + e.ifr_actual : ""),
                "M" === e.importance && (t = t.replace("@mark", '<i class="s1"></i><i class="s1"></i><i class="s0"></i><i class="s0"></i>')),
                "L" === e.importance && (t = t.replace("@mark", '<i class="s1"></i><i class="s0"></i><i class="s0"></i><i class="s0"></i>')),
                "H" === e.importance && (t = t.replace("@mark", '<i class="s1"></i><i class="s1"></i><i class="s1"></i><i class="s0"></i>'))
            }),
            this.$elem.html(t)
        },
        error: function(e) {},
        ...
        next: function() {
            ...
            e.animate({
                ...
                e.css("margin-top", 0).children("div").first().appendTo(e)
            })
        },
        infinite: function() {
